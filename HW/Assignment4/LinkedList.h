...
public:
    ...
    // to the first BigInt
    LinkedList &operator+=(LinkedList &rhs);
    void PrintWord(std::string positions);
};
#endif /* LinkedList_h */
...
template<class T>
void LinkedList<T>::PutItem(T newEntry)
{
...
template<typename T>
void LinkedList<T>::PrintWord(std::string positions)
{
    if (!positions.empty()) {
        ...
        size_t next = 0;
        int pos = 0;
        while ((next = positions.find(",", last)) != std::string::npos)
        {
            int x = std::stoi(positions.substr(last, next - last));
            pos += x;
            if(pos < _size)
                std::cout << Get(pos);
            last = next + 1;
